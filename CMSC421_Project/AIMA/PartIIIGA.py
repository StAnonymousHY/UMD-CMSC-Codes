...
def genetic_algorithm_tsp(dist, mutation_chance, population_size, num_generations, tournament_k, trace=None):
    n = dist.shape[0]
    population = [random_tour(n) for _ in range(population_size)]

    # initial best
    if trace is not None:
        best_now = max(population, key=lambda t: score(dist, t))
        trace.append(tour_cost(dist, best_now))
    ...
    for _ in range(num_generations):
        ...
        if trace is not None:
            best_now = population[0]  # because sorted best-first
            trace.append(tour_cost(dist, best_now))
...
csv_out = sys.argv[6] if (len(sys.argv) > 6) else "ga_trace.csv"
trace = []
tour, cost = genetic_algorithm_tsp(dist, mutation_chance, population_size, num_generations, tournament_k, trace=trace)
