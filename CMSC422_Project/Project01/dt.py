    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                # We can do this because X and Y are numpy.array
            ...
            if bestFeature < 0:
                ...
                ### TODO: YOUR CODE HERE
                leftIdx  = X[:,bestFeature] < 0.5
                rightIdx = X[:,bestFeature] >= 0.5
                # newUsed = used + [bestFeature]
                used.append(bestFeature)
                self.left.trainDT(X[leftIdx,:],  Y[leftIdx],  maxDepth-1, used.copy())
                self.right.trainDT(X[rightIdx,:], Y[rightIdx], maxDepth-1, used.copy())
