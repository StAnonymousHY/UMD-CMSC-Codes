...
class MCTree:
        ...
        for n in self.tree.iterNodes():
            ...
            for i in range(len(Y)):
                if Y[i] in leftLabels:
                    thisXList.append(X[i])
                    thisYList.append(-1)
                if Y[i] in rightLabels:
                    thisXList.append(X[i])
                    thisYList.append(1)
            thisX = array(thisXList)
            thisY = array(thisYList)
    ...
    def predict(self, X):
        ...
        while not curr.isLeaf:
            probs = curr.getNodeInfo().predict_proba(X)
            curr = curr.getRight() if probs[0,1] > 0.5 else curr.getLeft()
