...
-- 上面是错的，在ghci中使用QC.quickCheck prop_revapp可以得到结果

prop_revapp_correct :: [Int] -> [Int] -> Bool
prop_revapp_correct xs ys = reverse (xs ++ ys) == reverse ys ++ reverse xs
...
--这个algorithm有错，因为它的lhs和rhs都只包含小于或者大于pivot的数值，等于pivot的数值没有包括
--在下面的prop_qsort_sort可以看到[3,3]是失败的例子
--或者说，我们这个quick sort只对distinct element lists起效
...
-- x/= 是一个infix operator，意思和 \y -> x/=y 是一样的
-- x/= 后面要跟一个东西，跟什么不知道，所以就和function一样在等parameter input
...
--这里就是判断如果传进来的list都是distinct values，那么我们的quicksort就和build-in的sort是一样的结果
-- ==> 这个标识是只有左端的条件成立，才会执行右端的操作
...
genMaybe ga = QC.oneof [return Nothing,                 --generator for Nothing
                        ga >>= (\a -> return (Just a))] --generator for Just
