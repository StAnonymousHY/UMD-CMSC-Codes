...
foldl1 :: (b -> a -> b) -> b -> [a] -> b
foldl1 _ z []     = z
foldl1 f z (x:xs) = foldl1 f (z `f` x) xs -- `f` makes f become an infix opertor function (same as f z x) ONLY WORKS FOR BINARY PARAMETERS
...
runTests :: IO ()
runTests = do
  _ <- runTestTT $ TestList [ testLength,  testAll, testLast, 
                testFilter,  testReverse, testIntersperse, testFoldl ]
  return ()
