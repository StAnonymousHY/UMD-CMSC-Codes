...
zipTree (Branch l1 l2) (Branch r1 r2) = Branch (zipTree l1 r1) (zipTree l2 r2)
zipTree _ _ = error "Trees should have the same shape"
...
testZip0 :: Bool
testZip0 =
  zipTree (Branch (Leaf "a") (Branch (Leaf "b") (Leaf "c"))) (Branch (Leaf (0 :: Int)) (Branch (Leaf 1) (Leaf 2)))
  ==
...
zipTree1 (Branch l1 l2) (Branch r1 r2) = case zipTree1 l1 r1 of
  Just l -> case zipTree1 l2 r2 of
...
{-
Monad 理解：
Monad主要的两个运算符：<- 和 return
这两个运算符都在同一个 do block 里面
<-      负责把需要的东西从container中提取出来 （x <- go l l'）表示将Tree (a,b) 从Maybe (Tree (a,b))中提取出来 
        有点像try， x和y要都不是Nothing才能return
        Each <- line extracts the value from a Maybe. 
        If either go l l' or go r r' evaluates to Nothing, 
        then the entire do block short-circuits and returns Nothing

return  负责把运算过后的东西装回container （return (Branch x y)）表示把Branch x y :: Tree (a,b) 封装回Maybe中重新变成 Mybe (Tree (a,b))

因为<-和return都在同一个do block中，所以Haskell知道return应该封装回哪个container中
-}
...
colorsNeeded = List.find (not . null . stateColors) cs where
