...
that the semantics of a program can be described mathematically. 
...
You can use your IDE (emacs/VSCode) to evaluate the following line: 

-}

-- >>> 3 * (4 + 5)
-- C:\Users\YHXHa\AppData\Local\Temp\ext4C09: withFile: resource busy (file is locked)
-- C:\Users\YHXHa\AppData\Local\Temp\ext4C09: withFile: resource busy (file is locked)
-- C:\Users\YHXHa\AppData\Local\Temp\ext4C09: withFile: resource busy (file is locked)
...
{- 
We can ask our IDE to calculate with these names, just as we did above.
-}
...
other tools are available.](https://www.cs.umd.edu/class/spring2024/cmsc433/resources.html)

First use the terminal to start `ghci` and instruct it to load the `Basics`
module.

    lemonidas@Lemonidas17:~/433/lectures-private/01-basics$ stack ghci Basics.hs
    Using configuration for cmsc433-basics:lib to load /home/lemonidas/433/lectures-private/01-basics/Basics.hs
    cmsc433-basics> configure (lib)
    Configuring cmsc433-basics-0.1.0.0...
    cmsc433-basics> initial-build-steps (lib)
    Configuring GHCi with the following packages: cmsc433-basics.
    GHCi, version 9.4.5: https://www.haskell.org/ghc/  :? for help
    [1 of 1] Compiling Basics           ( /home/lemonidas/433/lectures-private/01-basics/Basics.hs, interpreted )
     
    /home/lemonidas/433/lectures-private/01-basics/Basics.hs:129:10: warning: [-Woverflowed-literals]
        Literal 12345678901234567890 is out of the Int range -9223372036854775808..9223372036854775807
        |
    129 | bigInt = 12345678901234567890
        |          ^^^^^^^^^^^^^^^^^^^^
    Ok, one module loaded.
    Loaded GHCi configuration from /home/lemonidas/.cache/stack/ghci-script/eebac53e/ghci-script
    ghci>
...
actEx :: (IO String, String -> IO())
actEx = (getLine, \ s -> putStrLn("Hello " ++ s ++ "!"))   -- "\" defines an anonymous function
...
in the CS department. For the ones that we have no information about, we can
return `Nothing`.
-}
...
range i j = 
    if i > j then [] else i : (range(i + 1) j)  -- ":" list append. This is a recursive function
