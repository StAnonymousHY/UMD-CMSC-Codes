...
/** To verify programs, Dafny relies on the theory of Hoare triples. */
...
/** A _Hoare triple_ is a claim about the state before and
    ...
    We have already encountered these notions when specifying Dafny programs:
    they correspond to the requires and ensures clauses of methods!

    For example, consider the following Dafny method:
*/
method Example(x : int) returns (y : int)
  requires x == 0
  ensures y == 1 
{
  y := x + 1;
...
/* This correctness of this Dafny program stems from the validity of the 
Hoare triple:

    { x == 0 } y := x + 1; { y == 1 }

This triple states that the statement y := x + 1 would transform a state in
which x == 0 to a state in which y == 1. Note that multiple triples can be
valid for the same statement; for example, the following triple is also valid:

    { x == 0 } y := x + 1; { y == 1 && x == 0 }

What if our program consists of multiple instructions? */
method Example2(x : int) returns (y : int) 
  requires x == 0 
  ensures y == 2
{
  y := x + 1;
...
/* The correctness of this Dafny program stems from the validity of the 
Hoare triple:

  { x == 0 } y := x + 1; y := y + 1 { y == 2 }

Unfortunately, as programs grow larger and more complex, the validity of 
such triples becomes less and less obvious. Fortunately, Hoare triples
were designed to provide a compositional way of reasoning about programs.
As we will see, we will be able to break down the validity of a triple 
about a sequence of instructions to the validity of sequnce of trivial 
Hoare triples. For this example:
...
Which of the following Hoare triples are valid -- i.e., the claimed relation between P, c, and Q is true?
   ...
   4) {x == 2 && x == 3} x := 5 {x == 0}        trivially valid (PRECONDITION AND POSTCONDITION not satisfied)

   5) {true} {} {false}                         invalid (POSTCONDITION not satisfied)

   6) {false} {} {true}                         trivially valid (PRECONDITION not satisfied BUT IT IS VALID)

   7) {true} while true {} {false}              trivially valid (IF NEVER TERMINATE, THEN DON'T HAVE TO CHECK POSTCONDITION)

   8) {x == 0}          
        while x == 0 { x := x + 1; }            valid
      {x == 1}

   9) {x == 1}
        while x != 0 { x := x + 1; }            trivially valid (IF NEVER TERMINATE, THEN DON'T HAVE TO CHECK POSTCONDITION)
      {x == 100}
*/
...
/** * Proof Rules 
    ...
    If the goal of Hoare logic is to provide a _compositional_
    method for proving the validity of specific Hoare triples, 
    we want the structure of a program's correctness proof to
...
/** ** Assignment 
    ...
    Define Q[a/x] to mean "Q where a is substituted in
    place of x".
...
What would a hoare rule for 1-sided conditionals look like? （只有if没有else）
Fill in the following rule.
         {P &&  b} s1 {Q}  {P && !b} {} {Q}
      ----------------------------------------  (1-sided conditionals)
              {P} if b { s1 } {Q}
*/
...
method LoopToZero_3(m : int, p : int) returns (x: int, z: int) {
  //  { m >= 0 } ->
  //  { m >= 0 && p - m == p - m }
  x := m;
  //  { x >= 0 && p - x == p - m }
