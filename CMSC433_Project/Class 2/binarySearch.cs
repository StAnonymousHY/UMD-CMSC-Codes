...
// Command-line arguments: run --no-verify c:\Users\YHXHa\Desktop\UMD-CMSC-Codes\CMSC433_Project\Class 2\binarySearch.dfy
// binarySearch.dfy
...
method BinarySearch(a: array<int>, key: int) returns (n: int)
  ...
  ensures forall i: int {:trigger a[i]} :: 0 <= n < i < a.Length ==> key <= a[i]
  decreases a, key
{
  var lo, hi := 0, a.Length;
  while lo < hi
    invariant 0 <= lo <= hi <= a.Length
    invariant forall i: int {:trigger a[i]} :: 0 <= i < lo < a.Length ==> a[i] < key
    invariant forall i: int {:trigger a[i]} :: 0 <= hi < i < a.Length ==> key <= a[i]
    decreases hi - lo
  {
    var mid := (lo + hi) / 2;
    if a[mid] < key {
...
method Contains(a: array<int>, key: int) returns (present: bool)
  requires forall i: int, j: int {:trigger a[j], a[i]} :: 0 <= i < j < a.Length ==> a[i] <= a[j]
  ensures present == exists i: int {:trigger a[i]} :: 0 <= i < a.Length && key == a[i]
  decreases a, key
{
...
namespace _module {
  ...
  public partial class __default {
    ...
    public static bool Contains(BigInteger[] a, BigInteger key)
    {
      bool present = false;
      BigInteger _0_n;
      BigInteger _out0;
      _out0 = __default.BinarySearch(a, key);
      _0_n = _out0;
      present = ((_0_n) < (new BigInteger((a).Length))) && (((a)[(int)(_0_n)]) == (key));
      return present;
    }
  }
